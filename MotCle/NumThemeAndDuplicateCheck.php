<?php 
	...
	while ($CountNum = $IsNumLang->fetch()) {
		...
		if ($CountNum['COUNT(*)'] > 0) { 
			//Create NumMoCle si il y a déja au moins un motcle qui a la même langue que lui
			$Get = $DB->query('SELECT * FROM MOTCLE WHERE NumLang LIKE "'.$NumLang.'" LIMIT 1');
			while($Extract = $Get->fetch()){
				if ($CountNum['COUNT(*)'] < 9) {
					$NumMoCle = substr($Extract['NumMoCle'], 0, 6)."0".($CountNum['COUNT(*)']+1);
				}
				else{
					$Num = substr($Extract['NumMoCle'], 0, 6).($CountNum['COUNT(*)']+1);
				}
			}
		...
		//Create NumMoCle si il n'y a pas de MotCLe qui ont la meme langue que lui
